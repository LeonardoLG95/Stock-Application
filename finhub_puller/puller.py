...
class Puller:
    def __init__(self):
        ...
        self._timescale_driver = TimescaleDriver(log=self._log, host='localhost', port='5500')
    ...
    async def _get_symbols(self):
        ...
        self._stock_symbols = self._stock_symbols.union(self._finnhub_driver.get_crypto_symbols())
    ...
    def pull_tasks(self):
        ...
        self.create_task(function=self.get_and_persist_prices(self._resolutions[0]), days=1)
        self.create_task(function=self.get_and_persist_prices(self._resolutions[1]), weeks=1)
        self.create_task(function=self.get_and_persist_prices(self._resolutions[2]), weeks=4)
        self.create_task(function=self.get_and_persists_financial_reports('quarterly'), weeks=4)
        self.create_task(function=self.get_and_persists_financial_reports('annual'), weeks=4)
    ...
    async def _get_financial_reports(self, symbol: str, frequency: str):
        reports, concepts = await self._finnhub_driver.get_financial_report(symbol, frequency)
        if reports and concepts:
            asyncio.create_task(
                self._timescale_driver.persist_financial_reports(reports, concepts, frequency)
            )
