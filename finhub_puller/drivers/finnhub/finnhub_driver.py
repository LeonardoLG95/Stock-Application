...
class FinnhubDriver:
    ...
    async def get_symbols_of_index(self, index_symbol: str) -> set:
        response = await self._get(f"{self.url}{END_POINTS['index_endpoint'](index_symbol)}{API_TOKEN}")
        if response:
            return {symbol for symbol in response['constituents']}

    async def get_symbol_info(self, symbol: str) -> dict:
        info = await self._get(f"{self.url}{END_POINTS['stock_info_endpoint'](symbol)}{API_TOKEN}")
        if info and self._check_integrity_keys(tuple(info.keys()), EXPECTED_RESPONSE_KEYS['info_keys']):
    ...
    async def get_symbol_price(self, symbol, resolution) -> Tuple[dict]:
        ...
        if price and self._check_integrity_keys(tuple(price.keys()), EXPECTED_RESPONSE_KEYS['candle_keys']):
            return self._parse_symbol_price(symbol, resolution, price)

    async def _fetch_symbol_price(self, symbol: str, resolution: str):
        now = self._now()
        return await self._get(f"{self.url}{END_POINTS['stock_price_endpoint'](symbol, resolution, now)}{API_TOKEN}")

    def _parse_symbol_price(self, symbol: str, resolution: str, price: dict) \
            -> Tuple[dict]:
        parsed_prices = ()
        try:
    ...
    async def get_basic_financials(self, symbol: str) -> Tuple[dict]:
        basic_financials = await self._get(f"{self.url}{END_POINTS['stock_basic_financials'](symbol)}{API_TOKEN}")
    ...
    def _parse_basic_financials(self, symbol: str, financials: dict) -> Tuple[dict]:
        try:
            return tuple(self._parse_financial_time_window(symbol, financials[time_window], time_window)
                         for time_window in ('quarterly', 'annual'))

        except KeyError:
            self.log.error(f'Wrong basic financials response for symbol: {symbol}')

    def _parse_financial_time_window(self, symbol: str, financials: dict, time_window: str) -> dict:
        longest_column = None
        len_value = 0
        for column in financials:
            longest_column = column if len(financials[column]) > len_value else longest_column
            len_value = len(financials[column]) if len(financials[column]) > len_value else len_value
    ...
    async def get_financial_report(self, symbol: str, frequency: str):
        reports = await self._get(f"{self.url}{END_POINTS['stock_financial_reports'](symbol, frequency)}{API_TOKEN}")

        if reports.get('data') and self._check_integrity_keys(tuple(reports['data'][0].keys()),
                                                              EXPECTED_RESPONSE_KEYS['report_keys']):
            return self._parse_reports(reports, frequency)

        return None, None

    def _parse_reports(self, reports: dict, frequency: str) -> Tuple[Tuple[dict], Tuple[dict]]:
        parsed_reports = ()
        parsed_concepts = ()
    ...
    def _parse_report(self, report: dict, id_key: str, frequency: str) -> dict:
        ...
        for column, value in report.items():
            ...
            if column in ('startDate', 'endDate', 'filedDate', 'acceptedDate'):
                ...
                parsed_report[self._camel_to_snake(column)] = datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
            else:
